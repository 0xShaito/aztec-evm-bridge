...
function ensureEVMCompiled() {
  ...
  const outPath = join(evmPath, "out/L2Gateway7683.sol/L2Gateway7683.json")
...
async function main() {
  ensureEVMCompiled()
  logger.info("Deploying L2Gateway7683...")
  ...
  const artifact = getContractArtifact("L2Gateway7683")
  const encodedArgs = encodeAbiParameters(parseAbiParameters("address"), [config.evm.permit2Address as `0x${string}`])
